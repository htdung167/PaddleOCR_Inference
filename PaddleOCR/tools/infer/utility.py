...
def draw_ocr_box_txt(image,
    ...
    for idx, (box, txt) in enumerate(zip(boxes, txts)):
        x1, y1, x2, y2 = box
        box2 = np.array([[x1, y1], [x2, y1], [x2, y2], [x1, y2]], dtype=np.int32)
        if scores is not None and scores[idx] < drop_score:
            continue
        color = (random.randint(0, 255), random.randint(0, 255),
                 random.randint(0, 255))
        draw_left.polygon(box2, fill=color)
        img_right_text = draw_box_txt_fine((w, h), box2, txt, font_path)
        pts = np.array(box2, np.int32).reshape((-1, 1, 2))
