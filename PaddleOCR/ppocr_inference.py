...
def visualize_ocr(img, lst_box, lst_text, font_path="PaddleOCR/fonts/arial.ttf"):
    # Convert img from cv2 to PIL
    pil_img = convert_cvimg_to_pilimg(img=img)
    lst_box = np.array(lst_box)

    # Use PaddleOCR function to draw
    res_img = None 
    res_img = draw_ocr_box_txt(pil_img, lst_box, lst_text, font_path=font_path)
    res_img = cv.cvtColor(res_img, cv.COLOR_RGB2BGR)
    return res_img

def convert_cvimg_to_pilimg(img):
